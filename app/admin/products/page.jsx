...
//   const [imageFiles, setImageFiles] = useState([]);
//   const [videoFile, setVideoFile] = useState(null);
//   const [isVideoUploading, setIsVideoUploading] = useState(false); // Fixed: renamed from uploadingVideo
...
//   // ---------------- FIXED SUBMIT ----------------
// // In your ProductAdminPage component, fix these specific issues:

// // 1. Fix the video preview handling in handleSubmit:
// const handleSubmit = async () => {
//   if (!form.name || !form.category) {
//     showMessage("Product name and category are required", "error");
//     return;
...
//     // ✅ FIXED: Upload video if new file selected (only uploads actual file, not data URL)
//     if (videoFile && !video360.startsWith('data:video')) {
//       const videoUrl = await uploadVideo(videoFile);
//       if (videoUrl) {
//         uploadedVideo = videoUrl;
//       }
//     } else if (video360 && video360.startsWith('/uploads/')) {
//       // Keep existing uploaded video
//       uploadedVideo = video360;
//     }
...
//         }
//       }
//       uploadedImages = [...uploadedUrls, ...images.filter(img => 
...
//       video360: uploadedVideo, // This will be saved to database
//       services: form.services ? form.services.split(",").map(s => s.trim()).filter(s => s) : [],
//       features: form.features ? form.features.split(",").map(f => f.trim()).filter(f => f) : [],
//       availability: isStock ? "In Stock" : "Out of Stock"
//     };
...
// // 2. Fix the edit function to properly handle video:
// const handleEdit = (p) => {
//   setEditId(p._id);
//   setForm({
//     ...INITIAL_FORM,
...
//   // Set existing media (only if it's an actual uploaded file URL)
//   setThumbnail(p.thumbnail || "");
//   setImages(p.images || []);
  
//   // Only set video if it's a proper URL (not data URL)
//   if (p.video360 && (p.video360.startsWith('/uploads/') || p.video360.startsWith('http'))) {
//     setVideo360(p.video360);
//   } else {
//     setVideo360("");
//   }
...
//   fetchSubCategories(p.category?._id || p.category);
//   setActiveTab("basic");
//   window.scrollTo({ top: 0, behavior: "smooth" });
// };

// // 3. Fix the video display in the table:
// {product.video360 && (
...
//           {/* Form Content */}
//           <div className="space-y-6">
//             {/* Media Tab - Video Section (Moved up for testing) */}
//             {activeTab === "media" && (
//               <div className="space-y-8">
//                 {/* Thumbnail Section */}
//                 <div>
...
//                             />
//                             <div className="absolute top-2 left-2 bg-[#C08237] text-white px-2 py-1 rounded-full text-xs font-medium">
//                               <Play className="inline h-3 w-3 mr-1" /> NEW VIDEO
//                             </div>
//                           </>
//                         ) : (
//                           <>
...
//                           {videoFile?.name || (video360.startsWith('/') ? "Uploaded Video" : "360° View")}
//                         </div>
//                       </div>
//                     )}
//                   </div>
...
//                     </div>
//                   )}
//                   {video360 && !video360.startsWith('data:video') && !isVideoUploading && (
//                     <p className="mt-2 text-sm text-green-600">
//                       ✓ Video is ready: {video360.substring(video360.lastIndexOf('/') + 1)}
//                     </p>
//                   )}
//                 </div>
