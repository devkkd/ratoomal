...
//     // Get filtered subcategories for each category with search
//     const getFilteredSubCategories = (categoryId) => {
//         const searchTerm = searchTerms[categoryId]?.toLowerCase() || '';
...
//             // If subcategory is also in URL, select it
//             if (urlSubCategoryId) {
...
//             }
//         }
//     }, [urlCategory, urlCategoryId, urlSubCategoryId]);
...
// // Also update handleCategoryChange function for top tabs
// const handleCategoryChange = (category) => {
//     setActiveCategory(category);
    
//     // If category is "All Products", clear selections
...
// // Updated handleSidebarCategoryChange function
...
//             // Set navigation state instead of directly calling router.push
//             let url;
//             if (subCategoryId) {
//                 const subCategoryObj = allSubCategories.find(sub => sub._id === subCategoryId);
//                 url = `/category?category=${encodeURIComponent(categoryObj.name)}&id=${categoryId}&subcategory=${encodeURIComponent(subCategoryObj?.name || '')}&subid=${subCategoryId}`;
//             } else {
...
//                     return product.category === catId && product.subCategory === subCatId;
//                 } else {
//                     // FIX: Use catKey directly instead of catId
//                     return product.category === catKey;
//                 }
//             });
//         });
...
//     // Right sidebar filters - these remain the same
//     if (selectedFilters["finish/style"] && selectedFilters["finish/style"].length > 0 && !selectedFilters["finish/style"].includes("All Finishes")) {
//         filtered = filtered.filter(product =>
//             selectedFilters["finish/style"].some(finish => {
//                 if (finish === "All Finishes") return true;
...
//         // Reset URL to base category page
...
//                                                                     const isCategorySelected = selectedCategories.includes(category._id) && !selectedCategories.some(c => c.includes('-'));
                                                                    
//                                                                     return (
//                                                                         <label key={subCat._id} className="flex items-center gap-2 cursor-pointer">
//                                                                             <input
//                                                                                 type="checkbox"
//                                                                                 checked={isSelected || (isCategorySelected && !selectedCategories.some(c => c.includes('-')))}
...
//                                     <span> in <span className="font-semibold text-[#C08237]">{activeCategory}</span></span>
//                                 )}
//                             </p>
//                             {(selectedCategories.length > 0 || Object.values(selectedFilters).some(arr => arr.length > 0)) && (
//                                 <button
...
//                                 <p className="text-sm text-gray-500 mt-2">
//                                     {activeCategory !== "All Products" && (
//                                         <>Active category: <span className="font-semibold">{activeCategory}</span><br /></>
//                                     )}
//                                     Categories: {backendCategories.length} | 
//                                     Subcategories: {allSubCategories.length} |
//                                     Products: {products.length}
...
//     // Handle category change from top tabs
//     const handleCategoryChange = (category) => {
//         setActiveCategory(category);
...
//             const categoryObj = backendCategories.find(cat => cat.name === category);
//             if (categoryObj) {
//                 // Select the category and deselect any subcategories from it
...
//     // Updated handleSidebarCategoryChange function for multiple selection
//     const handleSidebarCategoryChange = (categoryId, subCategoryId = null) => {
//         const key = subCategoryId ? `${categoryId}-${subCategoryId}` : categoryId;
...
//                     newSelection = prev.filter(i => i !== key);
//                 } else {
//                     // Add this subcategory and remove the parent category if it exists
//                     newSelection = [
//                         ...prev.filter(i => i !== categoryId), // Remove parent category
//                         key // Add subcategory
//                     ];
//                 }
//             } else {
...
//                     // Remove parent category and all its subcategories
//                     newSelection = prev.filter(i => 
//                         i !== categoryId && !i.startsWith(`${categoryId}-`)
//                     );
//                 } else {
//                     // Add parent category and remove all its subcategories
//                     newSelection = [
//                         ...prev.filter(i => !i.startsWith(`${categoryId}-`)), // Remove all subcategories
//                         categoryId // Add parent category
//                     ];
//                 }
//             }
...
//                 if (hasSubCategories) {
//                     // If subcategories selected, navigate with first subcategory
...
//                     url = `/category?category=${encodeURIComponent(categoryObj.name)}&id=${categoryId}`;
//                 } else {
//                     // If nothing selected from this category
//                     url = '/category';
//                 }
...
//                         return product.category === catId && product.subCategory === subCatId;
//                     } else {
//                         // Parent category selected - show all products in this category
//                         return product.category === catKey;
//                     }
//                 });
//             });
...
//                             <p className="text-gray-600">
//                                 Showing <span className="font-semibold">{sortedProducts.length}</span> products
//                                 {activeCategory !== "All Products" && activeCategory !== "Custom" && (
//                                     <span> in <span className="font-semibold text-[#C08237]">{activeCategory}</span></span>
//                                 )}
//                                 {selectedCategories.length > 0 && (
//                                     <span> ({selectedCategories.length} filter{selectedCategories.length > 1 ? 's' : ''} applied)</span>
//                                 )}
//                             </p>
//                             {(selectedCategories.length > 0 || Object.values(selectedFilters).some(arr => arr.length > 0)) && (
//                                 <button
...
//                                     )}
//                                     {selectedCategories.length > 0 && (
//                                         <>Selected filters: <span className="font-semibold">{selectedCategories.length}</span><br /></>
//                                     )}
//                                     Categories: {backendCategories.length} | 
//                                     Subcategories: {allSubCategories.length} |
//                                     Products: {products.length}
...
//         // Multiple ways to check for token
//         let token = null;
...
//         console.log('Login check - Token exists:', !!token, 'Logged in:', loggedIn);
...
//         // Setup interval to check login status periodically
...
//       // Also check login status when component mounts
//     useEffect(() => {
//         checkLoginStatus();
//     }, [checkLoginStatus]);
...
//                         const totalPagesCount = Math.ceil(transformedProducts.length / productsPerPage);
...
//             if (!isLoggedIn) {
//                 // For non-logged in users, restrict to first page only
//                 if (currentPage > 1) {
//                     // If user somehow got to page > 1, reset to page 1
//                     console.log('Non-logged in user trying to access page', currentPage, 'redirecting to page 1');
//                     setCurrentPage(1);
//                     startIndex = 0;
//                 }
//                 endIndex = Math.min(productsPerPage, sortedProducts.length);
//             } else {
//                 // For logged in users, show current page
//                 endIndex = Math.min(currentPage * productsPerPage, sortedProducts.length);
//             }
            
//             const currentProductsSlice = sortedProducts.slice(startIndex, endIndex);
//             setCurrentProducts(currentProductsSlice);
            
//             console.log('Current products slice:', {
//                 startIndex,
//                 endIndex,
//                 sliceLength: currentProductsSlice.length,
//                 isLoggedIn,
//                 currentPage
//             });
//         }
...
//         console.log('Page change requested:', pageNumber, 'Logged in:', isLoggedIn);
        
//         if (!isLoggedIn && pageNumber > 1) {
//             // Redirect to login page if not logged in and trying to access page > 1
//             router.push('/login');
//             return;
//         }
...
//                         disabled={isDisabled}
//                         className={`px-4 py-2 rounded-lg border text-sm font-medium ${currentPage === i 
//                             ? 'bg-[#C08237] text-white border-[#C08237]' 
//                             : isDisabled 
//                                 ? 'bg-gray-100 text-gray-400 border-gray-300 cursor-not-allowed' 
//                                 : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}
//                         title={isDisabled ? "Login to view more products" : `Page ${i}`}
//                     >
//                         {i}
//                     </button>
//                 );
...
//                 className={`px-3 py-2 rounded-lg border text-sm ${isNextDisabled 
//                     ? 'bg-gray-100 text-gray-400 border-gray-300 cursor-not-allowed' 
//                     : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}
//                 title={!isLoggedIn && currentPage === 1 ? "Login to view more products" : ""}
//             >
//                 Next
//             </button>
//         );
...
//                                             : Math.min(productsPerPage, sortedProducts.length)
//                                     }</span> of <span className="font-semibold">{sortedProducts.length}</span> products
//                                 {!isLoggedIn && (
...
//                                                             • <a href="/login" className="text-[#C08237] font-medium hover:underline">Login</a> to view more pages
//                                                         </span>
//                                                     )}
//                                                 </p>
//                                             </div>
