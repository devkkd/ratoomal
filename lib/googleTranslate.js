...
class GoogleTranslateService {
  ...
  // Get all languages from the native Google Translate select element
  getAvailableLanguages() {
    ...
    return options
      .map(option => ({
        code: option.value,
        name: option.text,
        flag: this.getFlagUrl(option.value)
      }))
      .filter(lang => lang.code !== ''); // Filter out empty default option
  }

  getFlagUrl(langCode) {
    const customMappings = {
      en: 'us', hi: 'in', zh: 'cn', ar: 'sa', ja: 'jp', ko: 'kr', he: 'il',
      el: 'gr', uk: 'ua', vi: 'vn', fa: 'ir', ur: 'pk', bn: 'bd', pa: 'in',
      te: 'in', ta: 'in', gu: 'in', mr: 'in', sq: 'al', hy: 'am', az: 'az',
      eu: 'es', be: 'by', bs: 'ba', ca: 'es', hr: 'hr', cs: 'cz', da: 'dk',
      et: 'ee', gl: 'es', ka: 'ge', hu: 'hu', is: 'is', ga: 'ie', lv: 'lv',
      lt: 'lt', mk: 'mk', ms: 'my', mt: 'mt', sr: 'rs', sk: 'sk', sl: 'si',
      sw: 'ke', cy: 'gb', yi: 'il'
    };
    const countryCode = customMappings[langCode] || langCode;
